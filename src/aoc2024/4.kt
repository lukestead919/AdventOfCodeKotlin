...
fun main() {
    ...
    fun part1() {
        input
            .mapIndexed { y, row ->
                row.mapIndexed { x, _ ->
                        Direction.entries.count { checkForWord(Point(x, y), it, "XMAS") }
                    }
                    .sum()
            }
            .sum()
    ...
    fun part2() {
        val directions =
            listOf(Direction.UP_LEFT, Direction.UP_RIGHT, Direction.DOWN_RIGHT, Direction.DOWN_LEFT)
        val adjacentDirections =
            directions.zipWithNext() + (directions.last() to directions.first())
            ...
            .mapIndexed { y, row ->
                row.mapIndexed { x, _ ->
                        adjacentDirections.count { (direction1, direction2) ->
                            isTheAInMas(Point(x, y), direction1) &&
                                isTheAInMas(Point(x, y), direction2)
                        }
                    }
                    .sum()
            }
