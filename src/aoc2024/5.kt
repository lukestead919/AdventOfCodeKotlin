...
fun main() {
    fun readInput(): Pair<List<Pair<Int, Int>>, List<List<Int>>> {
        val (rules, updates) = read2024Input("5").filter { it.isNotBlank() }.partition { it.contains("|") }
    ...
    fun part1() {
        updates.filter { update ->
            rules.all { rule ->
                updateObeysRule(update, rule)
            }
        }.map { midpoint(it) }.sum().println()
    }
